...
CREATE TABLE `borrowdetail` (
  ...
  `timeNo` enum('A','B','C') NOT NULL COMMENT '時間編號',
  ...
  CONSTRAINT `borrowdetail_ibfk_3` FOREIGN KEY (`timeNo`) REFERENCES `time` (`timeID`) ON DELETE NO ACTION ON UPDATE CASCADE,
...
CREATE TABLE `desk` (
  ...
  `deskType` enum('QQ','AA') NOT NULL DEFAULT 'QQ' COMMENT '書桌規格',
  `deskDist` char(1) NOT NULL COMMENT '書桌區域',
  PRIMARY KEY (`deskID`)
...
CREATE TABLE `time` (
  `timeID` enum('A','B','C') NOT NULL COMMENT '時段編號',
  `time` enum('9:00~12:00','12:00~15:00','15:00~18:00') NOT NULL COMMENT '時段',
